...
4. WebSocket 即時推送  4. WebSocket 即時主動
使用 Socket.IO 技術
收到 MQTT 訊息後立即推送到網頁
無需手動重新整理
MQTT 數據格式
...
sudo ufw allow 8080
問題 3：沒有數據顯示
檢查終端機是否有「✅ MQTT 連線成功」訊息
運行 test_mqtt_publish.py 發送測試數據
檢查網頁的「MQTT 狀態」指示燈是否為綠色
效能比較
項目	Streamlit	Flask  燒瓶
記憶體佔用	~300MB  約 300MB	~50MB  約 50MB
CPU 佔用	高	低
啟動速度	慢（5-10秒）	快（<1秒）
ARM 相容性	❌ 不相容  ❌ 不兼容	✅ 完全相容
即時更新	需重新整理	WebSocket 推送  WebSocket 主動體
結論
